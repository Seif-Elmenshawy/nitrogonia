...
app.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      tempData.push({ x: secondsSinceStart, y: data.Temperature });
      levelData.push({x: secondsSinceStart, y: data["Water Level"]})
      moistureData.push({x: secondsSinceStart, y:data["Soil Moisture"]})
    ...
    tempData.reverse();
    moistureData.reverse();
    waterLevelData.reverse();

    console.log("DOC COUNT:", snapshot.size);
    console.log("FIRST DOC:", snapshot.docs[0]?.data());
    console.log("TEMP DATA:", tempData.length);

    res.render('dashboard', { tempData,levelData, moistureData, fanState, solenoidState, docId, lastUpdated });
  } catch (error) {
...
app.get('/api/sensor-data', async (req, res) => {
  try {
    ...
    snapshot.forEach(doc => {
      ...
      tempData.push({ x, y: data.Temperature });
      levelData.push({x, y: data["Water Level"]})
      moistureData.push({x, y: data["Soil Moisture"]})
    });

    res.json({ tempData, levelData, moistureData, fbfanState, fbsolenoidState});
